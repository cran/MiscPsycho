...
function(params1, params2, A = 1, B = 0, control=list()){
   ...
   p1 <- p2 <- p3 <- p4 <- p5 <- p6 <- p7 <- p8 <- p9 <- p10 <- p11 <- p12 <- p13 <- p14 <- matrix(c(numeric(L*M)), ncol=M, nrow=L)
   for(i in 1:L){
	for(k in 1:M){
   ...
   score3.result <- sum((2*colSums(p3)^2) + (2 * (colSums(p1) - colSums(p2))) * ((colSums(p5) + colSums(p6)) - colSums(p7)) * whts)
   score4.result <- sum((2 * colSums(p4)^2) + 2 * (colSums(p10) - colSums(p9)) * (colSums(p1) - colSums(p2)) * whts)
   score5.result <- sum((2 * colSums(p4)) * colSums(p3) + (2 * (colSums(p1) - colSums(p2))) *(colSums(p14) - (colSums(p12) + colSums(p13))) * whts)
   list(gradient = c(score1.result, score2.result), hessian = matrix(c(score3.result, score5.result,score5.result,score4.result), ncol=2))
}
